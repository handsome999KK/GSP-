...
def get_args_parser():
    ...
    parser.add_argument(
    '--test_ckpt_addr',
    default='pretrained_models/pretrained_models_ckpt_zero-sho_classification_pointbert_ULIP-2.pt',
    type=str,
    help='the ckpt to test 3d zero shot'
)

    parser.add_argument('--fullshot', action='store_true',
                    help='use scanobjectnn*_fps_fullshot.dat instead of default *_fps.dat')
    return parser
...
def test_zeroshot_3d_core(test_loader, model, tokenizer, args=None):
    ...
    if args.dataset_name == "ScanObjectNN15":
        ...
        validate_dataset_name = args.dataset_name
        with open(os.path.join("./data/SN", 'labels.json')) as f:
            labels = json.load(f)[validate_dataset_name]
    elif args.dataset_name == "ModelNet40":
        validate_dataset_name = args.dataset_name
        with open(os.path.join("./data/MN", 'labels.json')) as f:
            labels = json.load(f)[validate_dataset_name]
    else:
    ...
    with torch.no_grad():
        ...
        for l in labels:
            ...
            KMEAN = 3
            kmeans = KMeans(n_clusters=KMEAN, random_state=42, n_init=10)
            kmeans.fit(X)
    ...
    all_pc_features = all_pc_features / np.linalg.norm(all_pc_features, axis=1, keepdims=True)
    ...
    label_features = all_pc_features[test_num:]
    all_pc_features = all_pc_features[:test_num]    
    true_labels = np.array(true_labels).squeeze()
    label_labels = true_labels[test_num:]
    true_labels = true_labels[:test_num]


    pos = text_features.shape[0]
    neg = label_features.shape[0]
    
    N = all_pc_features.shape[0]
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    
    matrix1 = torch.eye(c, dtype=torch.float32, device=device)
    matrix1 = matrix1.repeat_interleave(KMEAN, dim=0)
    matrix2 = torch.zeros(c, N, dtype=torch.float32, device=device)
    matrix3 = np.zeros((c, len(label_labels)), dtype=int)
    matrix3 = torch.from_numpy(matrix3).to(device) 
    for idx, cls in enumerate(label_labels):
        matrix3[cls, idx] = 1
    for i in range(2):
        
        Adajency_Matrix_tensor = build_graph(text_features, all_pc_features, label_features)
        
        
        Y_array = torch.cat([matrix3, matrix1.T, matrix2], dim=1)
        Y_array_0 = Y_array.clone()
        a = 0.5
        n = int(np.ceil(np.log(N)))
        for _ in range(n):
            Y_array = a * (Adajency_Matrix_tensor @ Y_array.T).T + (1 - a) * Y_array_0
            Y_array[:, :pos + neg] = Y_array_0[:, :pos + neg]
        score_tensor = Y_array[:, -N:]
        score = score_tensor.cpu().numpy()
        
        
        matrix2 = self_train(score, matrix2)
        

    pred_top1 = np.argmax(score, axis=0)  

    top1_acc = np.mean(pred_top1 == true_labels)
    print("Top-1 accuracy: {:.4f}".format(top1_acc))

    K = min(5, score.shape[0])  

    topk_idx = np.argsort(score, axis=0)[-K:, :]   

    correct_topk = np.any(topk_idx == true_labels[None, :], axis=0) 

    top5_acc = np.mean(correct_topk)
    print("Top-5 accuracy: {:.4f}".format(top5_acc))




def build_graph(text_features, all_pc_features, label_features, KNN=10, text_knn=1):
    ...
    sparse_matrix = np.zeros((num_samples, num_samples), dtype=float)

    for i in range(num_samples):
        for j in range(KNN):
            neighbor_index = indices[i][j]
            euclidean_distance = np.linalg.norm(
                all_pc_features[i] - all_pc_features[neighbor_index]
            )
            sparse_matrix[i, neighbor_index] = np.exp(-euclidean_distance)

    S = csr_matrix(sparse_matrix).toarray()   
    dist_text_pc = np.linalg.norm(
        text_features[:, np.newaxis, :] - all_pc_features[np.newaxis, :, :],
        axis=2
    ) 
    sorted_indices_text = np.argsort(dist_text_pc, axis=0)  
    filtered_text_pc = np.zeros_like(dist_text_pc)

    for j in range(dist_text_pc.shape[1]):  
        topk_idx = sorted_indices_text[:text_knn, j]  
        filtered_text_pc[topk_idx, j] = dist_text_pc[topk_idx, j]
    mask_text = filtered_text_pc > 0
    filtered_text_pc[mask_text] = np.exp(-filtered_text_pc[mask_text])

    text_pc = filtered_text_pc 
    dist_label_pc = np.linalg.norm(
        label_features[:, np.newaxis, :] - all_pc_features[np.newaxis, :, :],
        axis=2
    ) 
    sorted_indices_label = np.argsort(dist_label_pc, axis=0)  
    filtered_label_pc = np.zeros_like(dist_label_pc)

    for j in range(dist_label_pc.shape[1]):  
        topk_idx = sorted_indices_label[:text_knn, j]  
        filtered_label_pc[topk_idx, j] = dist_label_pc[topk_idx, j]

    mask_label = filtered_label_pc > 0
    filtered_label_pc[mask_label] = np.exp(-filtered_label_pc[mask_label])

    label_pc = filtered_label_pc  
    L = label_features.shape[0]
    T = text_features.shape[0]
    N = all_pc_features.shape[0]
    zero_LL = np.zeros((L, L))
    zero_LT = np.zeros((L, T))
    zero_TL = np.zeros((T, L))
    zero_TT = np.zeros((T, T))
    top_row1 = np.hstack((zero_LL, zero_LT, label_pc))  
    top_row2 = np.hstack((zero_TL, zero_TT, text_pc))   
    top = np.vstack((top_row1, top_row2))               

    bottom = np.hstack((label_pc.T, text_pc.T, S))     

    expand_S = np.vstack((top, bottom)) 
    
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    expanded_S_tensor = torch.from_numpy(expand_S).float().to(device)
    W_tensor = expanded_S_tensor + expanded_S_tensor.t()
    d_tensor = expanded_S_tensor.sum(dim=1)
    d_tensor = torch.clamp(d_tensor, min=1e-12)
...
def self_train(score, matrix2, s = 1, reserve = 5):
    k, N = score.shape
    

    c = k // s   
    min_vals = np.zeros((c, reserve), dtype=float)
    max_vals = np.zeros((c, reserve), dtype=float)
    min_col_idx = np.zeros((c, reserve), dtype=int)
    max_col_idx = np.zeros((c, reserve), dtype=int)
    matrix2[:] = 0.0
    ...
    for i in range(c):
        ...
        flat = block.reshape(-1).astype(float)
        mu = flat.mean()
        sigma = flat.std()
        if sigma == 0:
            Z = flat - mu
        ...
        r = min(reserve, flat.size)
        flat_min_idx = np.argpartition(flat, r - 1)[:r]
        order_min = np.argsort(flat[flat_min_idx])
        flat_min_idx = flat_min_idx[order_min]

        min_vals[i, :r] = flat[flat_min_idx]
        min_col_idx[i, :r] = flat_min_idx % N
        flat_max_idx = np.argpartition(flat, -r)[-r:]
        order_max = np.argsort(-flat[flat_max_idx])
        flat_max_idx = flat_max_idx[order_max]

        max_vals[i, :r] = flat[flat_max_idx]
        max_col_idx[i, :r] = flat_max_idx % N
        for t in range(r):
            col = min_col_idx[i, t]
            val = min_vals[i, t]
            matrix2[start:end, col] = val  
        for t in range(r):
            col = max_col_idx[i, t]
            val = max_vals[i, t]
            matrix2[start:end, col] = val  
