        ...
        if self.process_data:
            if not os.path.exists(self.save_path):
                ...
                if self.generate_from_raw_data:
                    print_log('Processing data %s (only running in the first time)...' % self.save_path, logger='ScanObjectNN')
...
class ScanObjectNN(data.Dataset):
    def __init__(self, config):
        ...
        self.catfile = os.path.join(self.root, 'scanobjectnn15_shape_names.txt')
        ...
        shape_ids['train'] = [line.rstrip() for line in open(os.path.join(self.root, 'scanobjectnn15_train.txt'))]
        shape_ids['test'] = [line.rstrip() for line in open(os.path.join(self.root, 'scanobjectnn15_test.txt'))]

        assert (split == 'train' or split == 'test')
        shape_names = ['_'.join(x.split('_')[0:-1]) for x in shape_ids[split]]
        self.datapath = [(shape_names[i], os.path.join(self.root, shape_names[i], shape_ids[split][i]) + '.txt') for i
                         in range(len(shape_ids[split]))]
        print_log('The size of %s data is %d' % (split, len(self.datapath)), logger='ScanObjectNN')

        flag_fullshot = bool(getattr(getattr(config, "args", None), "fullshot", False))

        if self.uniform:
            suffix = "_fps_fullshot.dat" if flag_fullshot else "_fps.dat"
            self.save_path = os.path.join(
                self.root,
                'scanobjectnn%d_%s_%dpts%s' % (self.num_category, split, self.npoints, suffix)
            )
        else:
            self.save_path = os.path.join(
                self.root,
                'scanobjectnn%d_%s_%dpts.dat' % (self.num_category, split, self.npoints)
            )
        ...
        if self.process_data:
            if not os.path.exists(self.save_path):
                ...
                if self.generate_from_raw_data:
                    ...
                    self.save_path = os.path.join(
                        self.root,
                        'scanobjectnn%d_%s_%dpts%s' % (self.num_category, split, 8192, suffix)
                    )
                    print_log('Load processed data from %s...' % self.save_path, logger='ScanObjectNN')
                    print_log('since no exact points pre-processed dataset found and no raw data found, load 2048 pointd dataset first, then do fps to {} after, the speed is excepted to be slower due to fps...'.format(self.npoints), logger='ScanObjectNN')
                    with open(self.save_path, 'rb') as f:
                        self.list_of_points, self.list_of_labels = pickle.load(f)
        ...
        self.shape_names_addr = os.path.join(self.root, 'scanobjectnn15_shape_names.txt')
        with open(self.shape_names_addr) as file:
            lines = file.readlines()
            lines = [line.rstrip() for line in lines]
        self.shape_names = lines
...
class ShapeNetCore(data.Dataset):
    def __init__(self, config):
        ...
        self.catfile = os.path.join(self.root, 'shapenetcore54_shape_names.txt')
        ...
        shape_ids['train'] = [line.rstrip() for line in open(os.path.join(self.root, 'shapenetcore54_train.txt'))]
        shape_ids['test'] = [line.rstrip() for line in open(os.path.join(self.root, 'shapenetcore54_test.txt'))]
        ...
        if self.uniform:
            ...
            self.save_path = os.path.join(
                self.root,
                'shapenetcore%d_%s_%dpts%s' % (self.num_category, split, self.npoints, suffix)
            )
        else:
            self.save_path = os.path.join(
                self.root,
                'shapenetcore%d_%s_%dpts.dat' % (self.num_category, split, self.npoints)
            )
        ...
        if self.process_data:
            if not os.path.exists(self.save_path):
                # make sure you have raw data in the path before you enable generate_from_raw_data=True.
                if self.generate_from_raw_data:
                    print_log('Processing data %s (only running in the first time)...' % self.save_path, logger='ShapeNetCore')
                    self.list_of_points = [None] * len(self.datapath)
                    self.list_of_labels = [None] * len(self.datapath)
                    ...
                    self.save_path = os.path.join(
                        self.root,
                        'shapenetcore%d_%s_%dpts%s' % (self.num_category, split, 8192, suffix)
                    )

                    print_log('Load processed data from %s...' % self.save_path, logger='ShapeNetCore')
                    print_log('since no exact points pre-processed dataset found and no raw data found, load 4096 pointd dataset first, then do fps to {} after, the speed is excepted to be slower due to fps...'.format(self.npoints), logger='ShapeNetCore')
                    with open(self.save_path, 'rb') as f:
                        self.list_of_points, self.list_of_labels = pickle.load(f)
        ...
        self.shape_names_addr = os.path.join(self.root, 'shapenetcore54_shape_names.txt')
        with open(self.shape_names_addr) as file:
            lines = file.readlines()
            lines = [line.rstrip() for line in lines]
        self.shape_names = lines
...
class S3DIS(data.Dataset):
    def __init__(self, config):
        ...
        self.catfile = os.path.join(self.root, 's3dis7_shape_names.txt')
        ...
        shape_ids['train'] = [line.rstrip() for line in open(os.path.join(self.root, 's3dis7_train.txt'))]
        shape_ids['test'] = [line.rstrip() for line in open(os.path.join(self.root, 's3dis7_test.txt'))]
        ...
        if self.uniform:
            self.save_path = os.path.join(self.root,
                                          's3dis%d_%s_%dpts_fps.dat' % (self.num_category, split, self.npoints))
        else:
            self.save_path = os.path.join(self.root,
                                          's3dis%d_%s_%dpts.dat' % (self.num_category, split, self.npoints))

        if self.process_data:
            if not os.path.exists(self.save_path):
                # make sure you have raw data in the path before you enable generate_from_raw_data=True.
                if self.generate_from_raw_data:
                    print_log('Processing data %s (only running in the first time)...' % self.save_path, logger='S3DIS')
                    self.list_of_points = [None] * len(self.datapath)
                    self.list_of_labels = [None] * len(self.datapath)
                    ...
                    # no pre-processed dataset found and no raw data found, then load 8192 points dataset then do fps after.
                    self.save_path = os.path.join(self.root,
                                                  's3dis%d_%s_%dpts_fps.dat' % (
                                                  self.num_category, split, 8192))
                    print_log('Load processed data from %s...' % self.save_path, logger='S3DIS')
                    print_log('since no exact points pre-processed dataset found and no raw data found, load 4096 pointd dataset first, then do fps to {} after, the speed is excepted to be slower due to fps...'.format(self.npoints), logger='S3DIS')
                    with open(self.save_path, 'rb') as f:
                        self.list_of_points, self.list_of_labels = pickle.load(f)
        ...
        self.shape_names_addr = os.path.join(self.root, 's3dis7_shape_names.txt')
        with open(self.shape_names_addr) as file:
            lines = file.readlines()
            lines = [line.rstrip() for line in lines]
        self.shape_names = lines
