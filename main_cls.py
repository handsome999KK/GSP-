...
def test_zeroshot_3d_core(test_loader, model, tokenizer, args=None):
    ...
    matrix1 = torch.eye(c * KMEAN, dtype=torch.float32, device=device)
    matrix2 = torch.zeros(c * KMEAN, N, dtype=torch.float32, device=device)
    for i in range(2):
        
        Adajency_Matrix_tensor = build_graph(text_features, all_pc_features)
        
        
        Y_array = torch.cat([matrix1, matrix2], dim=1)
        Y_array_0 = Y_array.clone()
        a = 0.5
        n = int(np.ceil(np.log(N)))
        for _ in range(n):
            Y_array = a * (Adajency_Matrix_tensor @ Y_array.T).T + (1 - a) * Y_array_0
            Y_array[:pos + neg, :pos + neg] = Y_array_0[:pos + neg, :pos + neg]
        score_tensor = Y_array[:, -N:]
        score = score_tensor.cpu().numpy()
        
    
        
        matrix2 = self_train(score, KMEAN, matrix2)
        
        
    acc1, acc5 = calculate_accuracy(KMEAN, score, true_labels)
    print(f"Accuracy1: {acc1:.4f}")
    print(f"Accuracy5: {acc5:.4f}")

def calculate_accuracy(KMEAN, score, true_labels):
    score = np.asarray(score)
    true_labels = np.asarray(true_labels)
    k, N = score.shape

    c = k // KMEAN
    score_reshaped = score.reshape(c, KMEAN, N)

    score_new = score_reshaped.max(axis=1)   # (c, N)

    # ---------- Top-1 accuracy ----------
    pred_top1 = np.argmax(score_new, axis=0).astype(int)  # (N,)
    correct_top1 = np.sum(pred_top1 == true_labels)
    top1_acc = correct_top1 / N

    # ---------- Top-5 accuracy ----------
    top_k = 5
    top5_indices = np.argsort(score_new, axis=0)[-top_k:, :]

    correct_top5 = 0
    for i in range(N):
        if true_labels[i] in top5_indices[:, i]:
            correct_top5 += 1
    top5_acc = correct_top5 / N

    return top1_acc, top5_acc

def build_graph(text_features, all_pc_features, KNN=10, text_knn=1):
    ...
    filtered_matrix = np.zeros_like(dist_matrix)
    for j in range(dist_matrix.shape[1]):
        topk_idx = sorted_indices[:text_knn, j]
        filtered_matrix[topk_idx, j] = dist_matrix[topk_idx, j]
    filtered_matrix[filtered_matrix > 0] = np.exp(-filtered_matrix[filtered_matrix > 0])
    dist_matrix = filtered_matrix
    top_left = np.zeros((text_features.shape[0], text_features.shape[0]))
    top = np.hstack((top_left, dist_matrix))
    bottom = np.hstack((dist_matrix.T, S))
...
def self_train(score, KMEAN, matrix2, reserve = 5):
    k, N = score.shape
