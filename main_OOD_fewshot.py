...
def test_zeroshot_3d_core(test_loader, model, tokenizer, args=None):
    ...
    if args.dataset_name == "ScanObjectNN15":
        ...
        if args.dataset_split == "SR1":
            ...
            print(f"error dataset split choice: {args.dataset_split}")
            sys.exit(1)

    if args.dataset_name == "ShapeNetCore54":
        spilt = 18 * KMEAN
        spilt = int(spilt)
        spilt2 = 36 * KMEAN
        spilt2 = int(spilt2)
        test_num = 5158
        if args.dataset_split == "SN1":
            text_features = text_features[:, :spilt]  # SN1
            true = [0 if label > 17 else 1 for label in true_labels]   # SN1
        elif args.dataset_split == "SN2":
            text_features = text_features[:, spilt:  spilt2]  # SN2
            true = [1 if 17 < num < 36 else 0 for num in true_labels]    # SN2
        elif args.dataset_split == "SN3":
            text_features = text_features[:, -spilt:]  # SN3
            true = [1 if label > 35 else 0 for label in true_labels]   # SN3
        else:
            print(f"error dataset split choice: {args.dataset_split}")
            sys.exit(1)
    ...
    label_true = true[test_num:]
    true = true[:test_num]
    
    label_true = np.array(label_true) 
    filtered_features = label_features[label_true == 1] 

    text_features = np.vstack([text_features, filtered_features])
    ...
    for i in range(2):
        ...
        pos_num, neg_num, text_features = self_train(score, max_positions, min_positions, all_pc_features, text_features, neg, pos, N)
...
def self_train(score, max_positions, min_positions, all_pc_features, text_features, neg, pos, N, reserve = 5):
    mu, sigma = np.mean(score), np.std(score)
    Z = (score - mu) / sigma
    score = np.tanh(Z)
    ...
    pos_num = [score[i] for i in max_indices]
    neg_num = [score[i] for i in min_indices]
    return pos_num, neg_num, text_features
...
if __name__ == '__main__':
    ...
    if not args.test_ckpt_addr:
        ...
        ds = (args.dataset_name or "").lower()
        if ds in ["shapenetcore54"]:
            args.test_ckpt_addr = os.path.join(
                ckpt_dir,
